...
module Day7 where
  ...
  data Rule = Rule Color (M.Map Color Int)
    deriving (Eq, Ord, Show)
  ruleColor (Rule c _)  = c
  ruleHolds (Rule _ hs) = hs

  data Hold = Hold Int Color
    deriving (Eq, Ord, Show)
  holdAmount (Hold n _) = n
  holdColor  (Hold _ c) = c

  rule = do c <- color
            str " bags contain "
            hs <- (str "no other bags." >> return M.empty) <|> holds
            return $ Rule c hs

  color = Color <$> many (one >>= check (`elem` alphabet))
    where alphabet = concat [[' '], ['a' .. 'z']]

  holds = do (Hold n c) <- hold
             (str ", " >> holds <&> (M.insert c n)) <|> (chr '.' >> return (M.singleton c n))

  hold = do n <- many digit >>= int
            chr ' '
            c <- color
            str " bags" <|> str " bag"
            return $ Hold n c

  rules = map fst . concat . map (parse rule)

  data R = R Color [R] [(R, Int)]
  colorOf (R c _ _) = c

  ruleset :: [Rule] -> M.Map Color R
  ruleset rs = M.fromList $ map (colorOf &&& id) translated
    where translated = map translate rs
          translate (Rule c hs) = R c (holders c) (map holdee $ M.assocs hs)
          holders c = [r| r@(R _ _ hs) <- translated, c `elem` map (colorOf . fst) hs]
          holdee (c,n) = (,n) $ fromJust $ L.find ((==c) . colorOf) translated

  debug = map repr . M.elems
    where repr (R c rs hs) = "R " ++ show c ++ " " ++ show (map colorOf rs) ++ " " ++ show (map (colorOf *** id) hs)

  containers :: Color -> M.Map Color R -> S.Set Color
  containers c m = maybe S.empty build $ M.lookup c m
    where build (R _ rs _) = S.fromList (map colorOf rs) `S.union` (S.unions $ map build rs)

  containees :: Color -> M.Map Color R -> M.Map Color Int
  containees c m = maybe M.empty (build 1) $ M.lookup c m
    where build n (R _ _ rs) = M.unionsWith (+) $ map (recurse n) rs
          recurse n (r,m) = M.singleton (colorOf r) (n * m) `M.union` build (n * m) r
 
  sum :: Num a => M.Map k a -> a
  sum = M.foldr' (+) 0

  xs = ["dotted blue bags contain 3 wavy bronze bags, 5 clear tomato bags.",
  ...
  ys = ["light red bags contain 1 bright white bag, 2 muted yellow bags."
  ...
  zs = ["shiny gold bags contain 2 dark red bags."
