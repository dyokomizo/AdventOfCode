module Day4 where
  ...
  munge = go [[]]
    where go ps     []      = reverse ps
          go ps     ("":ls) = go ([]:ps) ls
          go (p:ps) (l:ls)  = go ((p ++ " " ++ l):ps) ls

  fields :: String -> [String]
  fields = snd . foldr split ([],[])
    where split ' ' (f,fs) = ([],(f:fs))
          split c   (f,fs) = ((c:f),fs)

  valid p = L.and $ map ($fields p) [has "byr",has "iyr",has "eyr",has "hgt",has "hcl",has "ecl",has "pid"]
    where has f = L.or . map (L.isPrefixOf f)

  unborked = length . filter id . map valid . munge

  strict p = L.and $ map ($fields p) [is byr,is iyr,is eyr,is hgt,is hcl,is ecl,is pid]
  is p = L.or . map (not . null . parse p)

  superduper = length . filter id . map strict . munge

  byr = str "byr:" >> four digit >>= int >>= check (>=1920) >>= check (<=2002) >>= suffix end
  iyr = str "iyr:" >> four digit >>= int >>= check (>=2010) >>= check (<=2020) >>= suffix end
  eyr = str "eyr:" >> four digit >>= int >>= check (>=2020) >>= check (<=2030) >>= suffix end
  hgt = str "hgt:" >> (_cm <|> _in)
    where _cm = many digit >>= int >>= check (>=150) >>= check (<=193) >>= suffix (str "cm" >> end)
          _in = many digit >>= int >>= check (>=59)  >>= check (<=76)  >>= suffix (str "in" >> end)
  hcl = str "hcl:" >> chr '#' >> exactly 6 hexcode >>= suffix end
  ecl = str "ecl:" >> (str "amb" <|> str "blu" <|> str "brn" <|> str "gry" <|> str "grn" <|> str "hzl" <|> str "oth") >>= suffix end
  pid = str "pid:" >> exactly 9 digit >>= suffix end
